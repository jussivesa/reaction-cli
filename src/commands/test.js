...
export function test(yargs) {
  ...
  if (hasArgs && !onlyHasPort) {
    ...
    if (subCommands[1] === 'unit') {
      cmd += ' --settings test_settings.json --port 3031 --once --headless --driver-package dispatch:mocha';
      Log.info('Running unit tests command:');
    } else {
      cmd += ' --settings test_settings.json --port 3031 --once --full-app --headless --driver-package dispatch:mocha';
